...
class GitConan(ConanFile):
    ...
    build_requires = ["libtool/2.4.6", "openssl/1.1.1g","libcurl/7.64.1","expat/2.2.9"]
    ...
    def _commonCommandCompileGit(self, install):
        self.build_requires
        ldflags = "-L{0} -L{1} -L{2} -L{3}".format(self.deps_cpp_info["libtool"].rootpath+"/lib", self.deps_cpp_info["libcurl"].rootpath+"/lib",self.deps_cpp_info["openssl"].rootpath+"/lib",self.deps_cpp_info["expat"].rootpath+"/lib" )
        os.environ["PATH"] += ":"+self.deps_cpp_info["libcurl"].rootpath+"/bin"
        return "cd {0}/{1} && make  -j$(nproc)  {2} prefix={3} LDFLAGS=\"{4}\" OPENSSLDIR={5} CURLDIR={6} EXPATDIR={7}".format(
            self.source_folder,
